namespace :cph do
  ...
  task flights: :environment do
    ...
    get_page("https://www.cph.dk/en/flight-information/arrivals", "Arrival");

    Rails.logger.info "====================================================="
    Rails.logger.info "Import all departure flights of Copenhagen Airport's"
    get_page("https://www.cph.dk/en/flight-information/departures", "Departure");
  end
end
...
def mapped_flights(page, type)
  ...
  page.css(".stylish-table__row--body").each do |row|
    ...
    existing_flight = Flight.find_by_flight_no(flight[:flight_no])
